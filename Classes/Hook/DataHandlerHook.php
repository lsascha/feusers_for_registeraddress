<?php
namespace Lsascha\Feusersforregisteraddress\Hook;
 ...
 *  Copyright notice
 *
 *  (c) 2018 Sascha Löffler <lsascha@gmail.com>
 *
 *  All rights reserved
 *
    ...
    public function processDatamap_postProcessFieldArray($status, $table, $id, array &$fieldArray, DataHandler $pObj)
    {
        if ($table === 'fe_users' && $status === 'new') {
            if (empty($fieldArray['registeraddresshash'])) {
                ...
                $regHash = sha1($fieldArray['email'] . $rnd);

                $fieldArray['registeraddresshash'] = $regHash;
            }
        }
    }
}
